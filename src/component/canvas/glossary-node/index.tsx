  ...
  const onMouseLeave = useCallback((event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    GlossaryLinks
      ...
      .forEach(notionLink => {
        ...
        if (notionRelation?.classList.contains("app-relation-source")) {
          notionRelation?.classList.remove("app-relation-source");
        }
      });
  ...
  const onMouseEnter = useCallback((event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    GlossaryLinks
      ...
      .forEach(notionLink => {
        ...
        if (!notionRelation?.classList.contains("app-relation-source")) {
          notionRelation?.classList.add("app-relation-source");
        }
      });
  }, [idNumber]);
